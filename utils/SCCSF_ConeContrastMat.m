classdef SCCSF_ConeContrastMat < SCCSF
    ...
    methods
        ...
        function S = csf_freq_size_lum( obj, freq, area, color_dir, lum )
            ...
            Nl = length(lum);
            S_max = zeros(Nl,1, class(lum));
            f_max = zeros(Nl,1, class(lum));
            bw = zeros(Nl,1, class(lum));
            gamma = zeros(Nl,1, class(lum));
            %eta = zeros(Nl,1);
            
            if length(color_dir)==1 && Nl>1
                color_dir = ones(Nl,1)*color_dir;
            end
    ...
    methods( Static )
        ...
        function p = get_default_par()
            ...
            % Fitted on 14/08/2020 after adding 4-centres and fixing the bug
            % related to the wrong scaling of the LMS
            p.cm(1).S_max = [ 346170 5.64888 0.319293 1.8663e+06 7.57549e-05 ];
            p.cm(1).f_max = [ 2.12365 2289.53 0.186249 ];
            p.cm(1).bw = 1.09076;
            p.cm(1).gamma = 1.11387;
            p.cm(1).Ac_prime = 52.3184;
            p.cm(2).S_max = [ 439.738 22.4193 0.479041 ];
            p.cm(2).f_max = 0.119941;
            p.cm(2).bw = 2.70798;
            p.cm(2).gamma = 1.73378;
            p.cm(2).Ac_prime = 1.25741;
            p.cm(3).S_max = [ 11867.2 253.395 0.33456 ];
            p.cm(3).f_max = 7.74728e-08;
            p.cm(3).bw = 5.33398;
            p.cm(3).gamma = 1.47414;
            p.cm(3).Ac_prime = 0.28267;
            p.colmat = [ 0.00135758 0.210461 0.936788 1.13497 6.43059e-07 0.00631996 ];
            
            p.ds.xuqiang = 1.67697;
            p.ds.sw = 1.3611;
            p.ds.kim2013_ach = 1.75397;
            p.ds.kim2013_ch = 0.743978;
